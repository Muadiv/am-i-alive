...
async def get_db() -> aiosqlite.Connection:
    """Get the global database connection, creating it if necessary."""
    global _db
    if _db is None:
...
async def remove_notable_event(event_id: int) -> bool:
    ...
    db = await get_db()
    cursor = await db.execute("DELETE FROM notable_events WHERE id = ?", (event_id,))
    await db.commit()
    return cursor.rowcount > 0


async def get_notable_events(life_number: int = None, limit: int = 50) -> list:
    """Get notable events for chronicle."""
    db = await get_db()
    db.row_factory = aiosqlite.Row
    if life_number:
        query = """
...
async def get_recent_blog_posts_with_notable_status(limit: int = 20) -> list:
    """Get recent blog posts with a flag indicating if they're already notable."""
    db = await get_db()
    db.row_factory = aiosqlite.Row
...
async def get_telegram_notifications(limit: int = 50) -> list:
